...
void setupIMU() {
	...
	delay(100); // for i2c stabilization
	imu.begin();
	configureIMU();
}
...
void configureIMU() {
	...
	imu.setRate(imu.RATE_50HZ_APPROX);
	imu.setupInterrupt();
}
...
/* void readIMU() {
    ...
    // Detectar si waitForData salió por error
    if (imu.status() != 0) {
        Serial.println("I2C bus error — reiniciando");
        Wire.end();
        delayMicroseconds(500);
        Wire.begin(I2C_SDA, I2C_SCL); // tus pines SDA, SCL
        Wire.setClock(100000);
        return; // saltar esta lectura, continuar el loop
    }
